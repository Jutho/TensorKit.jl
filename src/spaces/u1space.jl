...
conj(c::U1Charge)=U1Charge(c.charge)

immutable U1Space <: UnitaryRepresentationSpace{U1Charge}
    ...
    function U1Space(dims::Dict{U1Charge,Int},dual::Bool=false)
        ...
        ind=find(d.>0)
        isempty(find) && throw(ArgumentError("Dimension of a vector space should be bigger than zero"))
        new([c=>dims[c] for c in charges[ind]],dual)
    end
end

# Corresponding methods:
sectors(V::U1Space) = keys(V.dims)

dim(V::U1Space) = sum(values(V.dims))
dim(V::U1Space,c::U1Charge) = get(V.dims,c,0)
dual(V::U1Space) = U1Space(V.dims, !V.dual)
...
function directsum(V1::U1Space, V2::U1Space)
    ...
    for c1 in sectors(V1), c2 in sectors(V2)
        dims[c1*c2]+= dim(V1,c1)+dims(V2,c2)
    end
