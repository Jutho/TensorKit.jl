...
specified by specifying that the new sector at position `i` was originally at position
`p[i]` and assinging to every index a distinct level or depth `levels[p[i]]` (in other
words, `levels` lists depths of the indices in the order they are in before braiding). This
