...
_FusionStyle(::Type{T}) where {T<:SectorTuple} = FusionStyle(Base.tuple_type_head(T)) & _FusionStyle(Base.tuple_type_tail(T))

BraidingStyle(::Type{<:ProductSector{T}}) where {T<:SectorTuple} = _BraidingStyle(T)
_BraidingStyle(::Type{Tuple{}}) = Bosonic()
_BraidingStyle(::Type{T}) where {T<:SectorTuple} = BraidingStyle(Base.tuple_type_head(T)) & _BraidingStyle(Base.tuple_type_tail(T))
...
LinearAlgebra.:×(P1::ProductSector, P2::ProductSector) = ProductSector(tuple(P1.sectors..., P2.sectors...))

LinearAlgebra.:×(G1::Type{ProductSector{Tuple{}}}, G2::Type{ProductSector{T}}) where {T<:SectorTuple} = G2
LinearAlgebra.:×(G1::Type{ProductSector{T1}}, G2::Type{ProductSector{T2}}) where {T1<:SectorTuple,T2<:SectorTuple} =
    tuple_type_head(T1) × (ProductSector{tuple_type_tail(T1)} × G2)
LinearAlgebra.:×(G1::Type{ProductSector{Tuple{}}}, G2::Type{<:Sector}) = ProductSector{Tuple{G2}}
LinearAlgebra.:×(G1::Type{ProductSector{T}}, G2::Type{<:Sector}) where {T<:SectorTuple} = Base.tuple_type_head(T) × (ProductSector{Base.tuple_type_tail(T)} × G2)
LinearAlgebra.:×(G1::Type{<:Sector}, G2::Type{ProductSector{T}}) where {T<:SectorTuple} = ProductSector{Base.tuple_type_cons(G1,T)}
