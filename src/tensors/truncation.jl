...
truncbelow(epsilon::Real) = TruncationCutoff(epsilon)
...
function _truncate!(v::AbstractVector, trunc::TruncationDimension, p::Real = 2)
    ...
    truncerr::S = norm(view(v, dtrunc+1:length(v)), p)
    resize!(v, dtrunc)
    return v, truncerr
end
...
# For SectorDict
const SectorVectorDict{G<:Sector} = SectorDict{G,<:AbstractVector}

function _findnexttruncvalue(V::SectorVectorDict{G},
                                truncdim::SectorDict{G,Int}, p::Real) where {G<:Sector}
