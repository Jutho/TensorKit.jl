...
@testset "$(TensorKit.type_repr(I))" verbose = true for I in sectorlist
    ...
    @testset "Tensor Dict conversion ($T)" for T in (Int, Float32, ComplexF64)
        W = V1 ⊗ V2 ⊗ V3 ← V4 ⊗ V5
    ...
    if hasfusiontensor(I)
        @testset "Tensor Array conversion ($T)" for T in (Int, Float32, ComplexF64)
            W = V1 ⊗ V2 ⊗ V3 ← V4 ⊗ V5
            t = TensorMap(T == Int ? sz -> rand(-20:20, sz) : randn, W)
            a = @constinferred convert(Array, t)
            @test t ≈ @constinferred TensorMap(a, W)
        end
    end

    @testset "Basic linear algebra ($T)" for T in (Float32, ComplexF64)
        W = V1 ⊗ V2 ⊗ V3 ← V4 ⊗ V5
        t = TensorMap(rand, T, W)
        @test scalartype(t) == T
        @test space(t) == W
    ...
    if hasfusiontensor(I)
        @testset "Basic linear algebra: test via conversion ($T)" for T in
                                                                      (Float32, ComplexF64)
            W = V1 ⊗ V2 ⊗ V3 ← V4 ⊗ V5
            t = TensorMap(rand, T, W)
            t2 = TensorMap(rand, T, W)
        ...
        @timedtestset "Real and imaginary parts" for T in (Float64, ComplexF64, ComplexF32)
            W = V1 ⊗ V2
