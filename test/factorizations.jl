...
@testitem "left orth" setup = [Setup] begin
    ...
    algs = (TensorKit.QR(), TensorKit.QRpos(), TensorKit.QL(), TensorKit.QLpos(),
            TensorKit.Polar(), TensorKit.SVD(), TensorKit.SDD())

    testname(V) = "symmetry: $(TensorKit.type_repr(sectortype(first(V))))"
    @timedtestset "$(testname(V))" for V in spacelist
        W = ⊗(V...)
        for T in elts, alg in algs
            t = rand(T, W)
            test_leftorth(t, p, alg)
            tᴴ = t'
            test_leftorth(tᴴ, p, alg)
        end
    end
end
