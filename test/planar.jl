...
spacelist = Dict(FermionParity => GradedSpace{FermionParity}(0 => 1, 1 => 1),
                 FermionSpin => GradedSpace{FermionSpin}(0 => 2, 1 / 2 => 2, 1 => 1,
                                                         3 / 2 => 1),
                 FibonacciAnyon => GradedSpace{FibonacciAnyon}(:I => 2, :τ => 2),
                 IsingAnyon => GradedSpace{IsingAnyon}(:I => 2, :psi => 1, :sigma => 1),
)

@testset "BraidingTensor ($(TensorKit.type_repr(I)))" for I in keys(spacelist)
    V = spacelist[I]
    
    @testset "BraidingTensor conversion" begin
        for (V1, V2) in ((V, V), (V', V), (V, V'), (V', V'))
